...
class TactileDigit(DiscreteDevice):
    ...
    def __init__(self, device_id, name=None, start_frame_latency=0):
        super().__init__(device_id, name if name else f"Digit_{device_id}", start_frame_latency)
        self.formats = ['.png']
    ...
    def _setup_connect(self) -> None:
        ...
        if hasattr(self.handler, 'list_digits'):
            ...
            raise RuntimeError("Cannot enumerate DIGIT devices: no known handler method")
        if self.device_id not in serials:
            raise RuntimeError(f"DIGIT {self.device_id} not found (available: {serials})")
        self.sensor.connect()
