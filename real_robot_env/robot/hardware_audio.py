...
class AudioInput():
    def __init__(self, frames_per_buffer: int = 0, channels: int = 2, rate: int = 44100, audio_format: int = pyaudio.paInt16): #65536
        ...
        self.chunk_size = 1024 if frames_per_buffer == 0 else int(frames_per_buffer/8)  # Number of frames per buffer
    ...
    def get_state(self):
        ...
        try:
            ...
            if len(audio_data) >= target_length:
                ...
                print(f"DEBUG: Returning last {target_length} frames, shape: {result.shape}")
                return result
            else:
    ...
    def _continuous_capture(stop_event, is_stopped_event, device: pyaudio.PyAudio, audio_input: AudioInput, device_id, storage_queue: Queue) -> None:
        ...
        while not stop_event.is_set():
            ...
            if frame_count % 100 == 0:  # Print every 100 frames
                print(f"DEBUG: Captured {frame_count} chunks, total frames: {len(frames)}")
            ...
            try:
                ...
                if frame_count % 100 == 0:
                    print(f"DEBUG: Put frames into queue, queue size: {storage_queue.qsize()}")
            except Exception as e:
                if frame_count % 100 == 0:
